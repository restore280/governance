...
A voter who is the PR author, or who prefers not to use the GitHub review interface, may instead post a comment containing `/consent` on the PR. Such a comment constitutes that voter's written consent and electronic signature for purposes of Article VII of the Bylaws, with the same legal force and effect as a formal GitHub review approval. A voter may withdraw consent by posting `/dissent`; the most recent command controls. The timestamp and username recorded by GitHub for each comment constitute the date and identity of the signature.
